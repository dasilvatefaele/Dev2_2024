...
class Program
{
    static void Main(string[] args)
    {
        ...
        switch (scelta)
        {
            ...
            case "4":
                Console.Write("ID > ");
                int idProdotto2 = int.Parse(Console.ReadLine());
                ProdottoAdvanced prodottoTrovato2 = manager.TrovaProdotto(idProdotto2);
                if (prodottoTrovato2 != null)
                {
                    ...
                    Console.Write("Prezzo > ");
                    decimal prezzo2 = decimal.Parse(Console.ReadLine());
                    Console.Write("Giacenza > ");
                    int giacenza2 = int.Parse(Console.ReadLine());
                    manager.AggiungiProdotto(new ProdottoAdvanced {Id = idBackup, NomeProdotto = nome2, PrezzoProdotto = prezzo2, GiacenzaProdotto = giacenza2});
                }
                else
                {
                    Console.WriteLine($"\nProdotto non trovato per ID {idProdotto2}");
                }
            break;
        }
    }
