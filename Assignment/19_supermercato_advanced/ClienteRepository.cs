...
public class ClienteRepository
{
    ...
    public void SalvaProdotti(List<ProdottoAdvanced> prodotti)
    {
        ...
        {
            Directory.CreateDirectory(dirCarrello); // creala
        }
        ...
        foreach (var prodotto in prodotti) // per ogni prodotto nella lista argomento
        {
            ...
            nuovoPercorso = Path.Combine(dirCarrello,nomeProdotto); // creo il percorso
            File.WriteAllText(nuovoPercorso, jsonData); // scrivo il prodotto deserializzato nel percorso
        }
        
        // Console.WriteLine($"Dati salvati nella cartella '/{dirCarrello}'\n"); // stampo il percorso 
    }

    public List<ProdottoAdvanced> CaricaProdotti()
    {
        // string nuovoPercorso = Path.Combine(dirCarrello, prodo)
        if (Directory.Exists(dirCarrello))
        {
            string[] files = Directory.GetFiles(dirCarrello);  // carico il contenuto della cartella in una lista di stringhe
